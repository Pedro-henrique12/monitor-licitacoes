...
createApp({
    data() {
        return {
            ...
            listaUFs: [], 
            listaCidades: [], 
            coresSistemas: CORES_SISTEMAS,
            dadosRadar: [], 
            dadosRadarFiltrados: [], 
            radarTipoOrgao: 'Todos', 
            radarMeses: 2, 
            radarPlataforma: 'Todas', 
            listaPlataformasRadar: [],
            alertasExpandidos: false, 
            paginaAtualRadar: 1, 
            itensPorPagina: 50,
            
            // --- VARIÁVEIS EXCLUSIVAS DO PLANEJADOR DE ROTAS ---
            planejadorUF: '',
    ...
    computed: {
        ...
        cidadesFiltradasPlanejador() {
            ...
            // Filtra as cidades apenas do estado selecionado no planejador
            ...
            const termo = this.planejadorBuscaCidade.toLowerCase();
            return cidadesEstado.filter(c => c.toLowerCase().includes(termo));
        },
        calcularTotalKM() {
    ...
    methods: {
        ...
        async carregarArquivos() {
            try {
                // Carrega o Radar separadamente primeiro (para evitar que uma falha nele trave os outros)
                ...
                // Carrega os arquivos vitais em paralelo
        ...
        filtrarRadar() {
            ...
            if (this.radarPlataforma !== 'Todas') {
                filtrados = filtrados.filter(d => d.Plataforma === this.radarPlataforma);
            }
        ...
        limparSelecaoCidadePlanejador() {
            this.novaCidadeRota.municipio = '';
            this.planejadorBuscaCidade = '';
            this.novaCidadeRota.orgaosDisponiveis = [];
        },

        selecionarCidadePlanejador(cidade) {
            this.novaCidadeRota.municipio = cidade;
            this.novaCidadeRota.uf = this.planejadorUF;
            this.planejadorBuscaCidade = ''; // Limpa a busca ao selecionar
            
            // Busca os órgãos no Histórico
            const licitacoes = this.dadosHistorico.filter(d => 
                d.municipio === this.novaCidadeRota.municipio && d.uf === this.planejadorUF
            );
        ...
        adicionarCidadeARota() {
            ...
            // Limpa apenas a cidade e os valores. Mantém o estado (UF) para facilitar adicionar cidades próximas.
            this.novaCidadeRota = { 
                municipio: '', uf: this.planejadorUF, km_estrada: 0, km_cidade: 0, 
        ...
        async salvarNoBanco() {
            if(confirm("Confirmar envio do planejamento de " + this.rotaEmPlanejamento.passos.length + " cidades para o MySQL?")) {
                alert("Simulação: Rota enviada com sucesso para as tabelas 'rotas_planejamento' e 'rota_cidades_detalhes' do banco de dados Licitanet!");
            }
        },
